...
namespace DuckGame.OstrichMod
{
    ...
    public class Reaper : Gun
    {
        ...
        public Reaper(float xval, float yval) : base(xval, yval)
        {
            this.ammo = 4;
            ...
            this._type = "gun";
            base.graphic = new SpriteMap(Mod.GetPath<DuckGame.OstrichMod.OstrichMod>("Reaper"), 15, 29, false);
            this.center = new Vec2(4f, 21f);
            this.collisionOffset = new Vec2(-2f, -15f);
        ...
        public void Shing()
        {
            if (!this._shing)
            {
                ...
                if (!Reaper._playedShing)
                {
                    Reaper._playedShing = true;
        ...
        public override void Update()
        {
            ...
            else
            {
                ...
                if (this._volatile && this._hitWait == 0)
                {
                    ...
                    bool flag3 = false;
                    using (System.Collections.Generic.IEnumerator<Thing> enumerator = Level.current.things[typeof(Reaper)].GetEnumerator())
                    {
                        while (enumerator.MoveNext())
                        {
                            Reaper sword = (Reaper)enumerator.Current;
            ...
            if ((this._swung || this._swinging) && !this._shing)
            {
                ...
                if (!flag4)
                {
                    using (System.Collections.Generic.IEnumerator<Thing> enumerator4 = Level.current.things[typeof(Reaper)].GetEnumerator())
                    {
                        while (enumerator4.MoveNext())
                        {
                            Reaper sword2 = (Reaper)enumerator4.Current;
        ...
        public override void Draw()
        {
            ...
            if ((this.owner == null && base.velocity.length > 1f) || this._swing != 0f)
            {
                ...
                for (int i = 0; i < 7; i++)
                {
                    ...
                    base.depth -= 2;
                    base.alpha -= 0.15f;
                    base.graphic.color = Color.Black;
                }
