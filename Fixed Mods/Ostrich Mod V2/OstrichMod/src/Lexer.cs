...
namespace DuckGame.OstrichMod
{
    ...
    public class Lexer : Gun
    {
        ...
        public Lexer(float xval, float yval) : base(xval, yval)
        {
            this._editorName = "Lexer";
            this.ammo = 400;
            ...
            this._type = "gun";
            base.graphic = new SpriteMap(Mod.GetPath<DuckGame.OstrichMod.OstrichMod>("Lexer"), 11, 26, false);
            this.center = new Vec2(5f, 21f);
            this.collisionOffset = new Vec2(-3f, -21f);
            this.collisionSize = new Vec2(11f, 26f);
            this._barrelOffsetTL = new Vec2(5f, 13f);
            this._fireSound = GetPath("sounds/sawdown");
            this._fullAuto = true;
            this._fireWait = 1f;
        ...
        public void Shing()
        {
            if (!this._shing)
            {
                ...
                if (!Lexer._playedShing)
                {
                    Lexer._playedShing = true;
        ...
        public override void Update()
        {
            if(this.ammo > 0)
            {
                if (this._jabStance && this._swinging)
                {
                    this.ammo--;
            ...
            else
            {
                ...
                if (this._volatile && this._hitWait == 0)
                {
                    ...
                    bool flag3 = false;
                    using (System.Collections.Generic.IEnumerator<Thing> enumerator = Level.current.things[typeof(Lexer)].GetEnumerator())
                    {
                        while (enumerator.MoveNext())
                        {
                            Lexer sword = (Lexer)enumerator.Current;
            ...
            if ((this._swung || this._swinging) && !this._shing)
            {
                ...
                if (!flag4)
                {
                    using (System.Collections.Generic.IEnumerator<Thing> enumerator4 = Level.current.things[typeof(Lexer)].GetEnumerator())
                    {
                        while (enumerator4.MoveNext())
                        {
                            Lexer sword2 = (Lexer)enumerator4.Current;
