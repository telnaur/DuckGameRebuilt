using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using DuckGame;

namespace MyMod.src
{
    [EditorGroup("Zyrafa|Guns|Slo-Mo")]
    public class slomoMagnum : Gun
    {
        public StateBinding _angleOffsetBinding = new StateBinding("_angleOffset", -1, false, false);
        public StateBinding _riseBinding = new StateBinding("rise", -1, false, false);
        public float rise;
        public float _angleOffset;

        public override float angle
        {
            get
            {
                return base.angle + this._angleOffset;
            }
            set
            {
                this._angle = value;
            }
        }

        public slomoMagnum(float xval, float yval)
          : base(xval, yval)
        {
            this.ammo = 6;
            this._ammoType = (AmmoType)new slomoATMagnum();
            this._ammoType.immediatelyDeadly = true;
            this._type = "gun";
            this.graphic = new Sprite(GetPath("slomoMagnum"), 0.0f, 0.0f);
            this.center = new Vec2(16f, 16f);
            this.collisionOffset = new Vec2(-8f, -6f);
            this.collisionSize = new Vec2(16f, 10f);
            this._barrelOffsetTL = new Vec2(25f, 12f);
            this._fireSound = "magnum";
            this._kickForce = 5f;
            this._holdOffset = new Vec2(1f, 2f);
            this.handOffset = new Vec2(0.0f, 1f);
            this._editorName = "Slo-Mo Magnum";
            this.editorTooltip = "These bullets are so slooow!";

        }

        public override void Update()
        {
            base.Update();
            this._angleOffset = this.owner == null ? 0.0f : ((int)this.offDir >= 0 ? -Maths.DegToRad(this.rise * 50f) : -Maths.DegToRad((float)(-(double)this.rise * 50.0)));
            if ((double)this.rise > 0.0)
                this.rise -= 0.013f;
            else
                this.rise = 0.0f;
            if (!this._raised)
                return;
            this._angleOffset = 0.0f;
        }

        public override void OnPressAction()
        {
            base.OnPressAction();
            if (this.ammo <= 0 || (double)this.rise >= 1.0)
                return;
            this.rise += 0.4f;
        }
    }
}
