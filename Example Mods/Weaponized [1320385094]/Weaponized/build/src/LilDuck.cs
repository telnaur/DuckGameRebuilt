...
namespace MyMod.src
{
    ...
    public class LilDuck : Gun
    {
        ...
        public LilDuck(float xval, float yval)
          : base(xval, yval)
        {
            this.ammo = 1;
            ...
            this.bouncy = 0.8f;
            this.friction = 0.05f;
            this._editorName = "Lil Quack Grenade";
            this.editorTooltip = "We decided to stuff some explosives in some ducks in some more ducks in one bite-sized duck.";
        }

        public override void Initialize()
        {
            this.gr = LilDuck.lilDuck;
            ++LilDuck.lilDuck;
        }
            ...
            if (!this._localDidExplode && (double)this._timer < 0.0)
            {
                if (this._explodeFrames < 0)
                {
                    for (int index = 0; index < 8; ++index)
                    {
                        ...
                        lilFeather.vSpeed = -2f + Rando.Float(-1f);
                ...
                else
                {
                    ...
                    if (this._explodeFrames == 0)
                    {
                        ...
                        if (this.isServerForObject)
                        {
                            ...
                            {
                                LilDuck2 lilDuck2 = new LilDuck2(x, y);
                                float num2 = (float)index / 9f;
                                lilDuck2.hSpeed = /*(float)((double)num2 * 5.0 - 2.5) * */Rando.Float(-3f, 3f);
                                lilDuck2.vSpeed = Rando.Float(-3f, -5f);
                                lilDuck2.PressAction();
                                Level.Add((Thing)lilDuck2);
        ...
        public override void OnPressAction()
        {
            ...
            this._pin = false;
            lilFeather lilFeather = lilFeather.New(this.x, this.y);
            lilFeather.hSpeed = (float)-this.offDir * (1.5f + Rando.Float(0.5f));
