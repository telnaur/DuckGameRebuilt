...
namespace MyMod.src
{
    internal class trolley : Block
    {
        ...
        public trolley(float xpos, float ypos, Vec2 travel)
          : base(xpos, ypos)
        {
            this._travel = travel;
            this._sprite = new SpriteMap(GetPath("trolley"), 98, 60, false);
            this.graphic = (Sprite)this._sprite;
            this._sprite.AddAnimation("idle", 1f, true, 0, 1);
            this._sprite.SetAnimation("idle");
            this.center = new Vec2(49f, 30f);
            this._collisionSize = new Vec2(98f, 60f);
            this._collisionOffset = new Vec2(-49f, -30f);
            this.depth = (Depth)(1f);
        ...
        public override void Update()
        {
            if (sound == false)
            {
                SFX.Play(GetPath("trolleySoundCom"), 1.4f, 0.0f, 0.0f, false);
                sound = true;
            }
            trolley trolley = this;
            trolley.position = trolley.position + this._travel * 3.8f;
            ...
            timer -= 0.1f;
            life -= 0.01f;
            if (this.life < 5f && this.sound2 == false)
            {
                sound2 = true;
                SFX.Play(GetPath("ohJeez"), 0.8f, 0.0f, 0.0f, false);
            }
            if (life <= 0)
            {
                if (toRight)
            ...
            {
                Vec2 vec2 = new Vec2(physicsObject.center.x, this.y - 45f);
