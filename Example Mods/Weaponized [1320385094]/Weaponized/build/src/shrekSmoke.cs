...
namespace MyMod.src
{
    public class shrekSmoke : Thing
    {
        ...
        public shrekSmoke(float xpos, float ypos)
          : base(xpos, ypos, (Sprite)null)
        {
            this.xscale = 0.10f + Rando.Float(0.10f);
            this.yscale = this.xscale;
            this.angle = Maths.DegToRad(Rando.Float(360f));
            this._fastGrow = 0.15f + Rando.Float(0.25f);
            this._angleInc = Maths.DegToRad(Rando.Float(2f) - 1f);
            this._scaleInc = 1f / 1000f + Rando.Float(1f / 1000f);
            this._fade = 0.015f + Rando.Float(1f / 1000f);
            this.move.x = Rando.Float(0.4f) - 0.2f;
            this.move.y = Rando.Float(0.4f) - 0.2f;
            GraphicList graphicList = new GraphicList();
            Sprite graphic1 = new Sprite(GetPath("shrekSmoke"), 0.0f, 0.0f);
            graphic1.depth = (Depth)1f;
            graphic1.CenterOrigin();
            graphicList.Add(graphic1);
            Sprite graphic2 = new Sprite(GetPath("shrekSmokeBack"), 0.0f, 0.0f);
            graphic2.depth = (Depth) 0.05f;
            graphic2.CenterOrigin();
            graphicList.Add(graphic2);
            this.graphic = (Sprite)graphicList;
            this.center = new Vec2(0.0f, 0.0f);
            this.depth = (Depth)1f;
            this._backgroundSmoke = new Sprite(GetPath("shrekSmokeBack"), 0.0f, 0.0f);
        }
