...
namespace MyMod.src
{
    ...
    public class trolleyHat : Gun
    {
        ...
        public trolleyHat(float xpos, float ypos)
          : base(xpos, ypos)
        {
            this.ammo = 99;
            ...
            this._sprite = new SpriteMap(GetPath("trolleyGun"), 16, 16, false);
            ...
            this._hat = new SpriteMap(GetPath("trolleyHat"), 32, 32, false);
            ...
            this._kickForce = 1f;
            this.flammable = 0.8f;
            this._editorName = "Trolley Hat";
            this.editorTooltip = "The cause of many moral dillemas, but this time there's no debate, just violence.";
        }
        ...
        public override void OnPressAction()
        {
            if (this.owner != null)
            {
                if (this.whistle == true)
                {
                    ...
                    trolley trolley = new trolley(this.x - 1300f * this.offDir, this.y - 18f, vector);
                    Level.Add((Thing)trolley);
                    for (int index = 0; index < 4; ++index)
                        Level.Add((Thing)SmallSmoke.New(this.x + Rando.Float(-2f, 2f), this.y /*+ 5f + Rando.Float(-2f, 2f)*/));
                    if (Network.isActive)
                    {
                        trolley.Fondle((Thing)trolley);
                    }
                    this.whistle = false;
