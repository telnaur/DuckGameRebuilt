using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace DuckGame.DeathcratesPlusPlus
{
    class DCXPresentExplosion : DeathCrateSetting
    {
        public override void Activate(DeathCrate c, bool server = true)
        {
            float cx = c.x;
            float cy = c.y - 2f;
            Level.Add(new ExplosionPart(cx, cy, true));
            int num = 6;
            if (Graphics.effectsLevel < 2)
            {
                num = 3;
            }
            for (int i = 0; i < num; i++)
            {
                float dir = (float)i * 60f + Rando.Float(-10f, 10f);
                float dist = Rando.Float(12f, 20f);
                ExplosionPart ins = new ExplosionPart(cx + (float)(System.Math.Cos((double)Maths.DegToRad(dir)) * (double)dist), cy - (float)(System.Math.Sin((double)Maths.DegToRad(dir)) * (double)dist), true);
                Level.Add(ins);
            }
            if (server)
            {
                for (int j = 0; j < 12; j++)
                {
                    Present p = new Present(c.x, c.y);
                    float norm = (float)j / 7f;
                    p.hSpeed = (-15f + norm * 30f) * Rando.Float(0.5f, 1f);
                    p.vSpeed = Rando.Float(-3f, -11f);
                    Level.Add(p);
                }
                for (int k = 0; k < 2; k++)
                {
                    DeathCrate p2 = new DeathCrate(c.x, c.y);
                    float norm2 = (float)k / 3f;
                    p2.hSpeed = (-10f + norm2 * 20f) * Rando.Float(0.5f, 1f);
                    p2.vSpeed = Rando.Float(-3f, -11f);
                    p2.activated = true;
                    Level.Add(p2);
                }
                Level.Remove(c);
            }
            Graphics.FlashScreen();
            SFX.Play("harp", 1f, 0f, 0f, false);
        }
    }
}
