...
{
    class DCXBananaParty : DeathCrateSetting
    {
        public override void Activate(DeathCrate c, bool server = true)
        {
            ...
            if (server)
            {
                ...
                for(int i = 0; i < 5; i++)
                {
                    BananaCluster k = new BananaCluster(c.x, c.y);
                    float norm = (float)i / 8f;
                    k.hSpeed = (-20f + norm * 40f) * Rando.Float(0.5f, 1f);
                    k.vSpeed = Rando.Float(-3f, -12f);
                    k.PressAction();
                    Level.Add(k);
                }
                Level.Remove(c);
