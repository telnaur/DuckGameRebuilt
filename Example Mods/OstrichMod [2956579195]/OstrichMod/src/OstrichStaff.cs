...
namespace DuckGame.OstrichMod
{
	...
	public class OstrichStaff : Gun
	{
		...
		{
			Mod.GetPath<DuckGame.OstrichMod.OstrichMod>("sounds/rainbow")
		});
		...
		public OstrichStaff(float xpos, float ypos) : base(xpos, ypos)
		{
			this._editorName = "Ostrich Staff";
			this.sprite = new SpriteMap(Mod.GetPath<DuckGame.OstrichMod.OstrichMod>("OstrichStick"), 9, 28, false);
			base.graphic = this.sprite;
			this._center = new Vec2(4f, 21f);
			this._collisionOffset = new Vec2(-1f, -22f);
			this._collisionSize = new Vec2(3f, 24f);
			this._barrelOffsetTL = new Vec2(4f, 1f);
			this._holdOffset = new Vec2(-4f, 4f);
			this.ammo = 1;
			this._weight = 3f;
			this.medusaBit = new Sprite(Mod.GetPath<DuckGame.OstrichMod.OstrichMod>("ostrichBit"), 0f, 0f);
			this.medusaBit.CenterOrigin();
			this.beam = Content.Load<Tex2D>(Mod.GetPath<DuckGame.OstrichMod.OstrichMod>("ostrichBeam.png"));
		...
		private void Metamorphosis(PhysicsObject physicsObject)
		{
			...
			Level.Add(new Present(physicsObject.x, physicsObject.y));
