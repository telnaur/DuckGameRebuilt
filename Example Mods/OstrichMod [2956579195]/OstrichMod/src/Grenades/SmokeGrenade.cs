...
namespace DuckGame.OstrichMod
{
    ...
    class SmokeGrenade : Gun
    {
        ...
        {
            this._editorName = "Smoke Grenade";
            this.ammo = 1;
            this._ammoType = new ATMin();
            this._type = "gun";
            this._sprite = new SpriteMap(Mod.GetPath<DuckGame.OstrichMod.OstrichMod>("SmokeGrenade"), 16, 16);
        ...
        public override void Update()
        {
            ...
            if (_timeBeforeExplote <= 0)
            {
                for (int i = 0; i < 30; i++)
                {
                    Level.Add(new Smoke(x, y, 9f + Rando.Float(1f)));
                }
                Level.Remove((this));
            }
        }
