...
namespace DuckGame.OstrichMod
{
    ...
    public class Sonyblade : Gun
    {
        ...
        public Sonyblade(float xval, float yval) : base(xval, yval)
        {
            this._editorName = "Sonyblade";
            this.ammo = 4;
            this._ammoType = new ATSniper();
            this._ammoType.range = 170f;
            this._type = "gun";
            base.graphic = new SpriteMap(Mod.GetPath<DuckGame.OstrichMod.OstrichMod>("Sonyblade_blue"), 5, 23, false);
        ...
        public void Shing()
        {
            if (!this._shing)
            {
                ...
                if (!Sonyblade._playedShing)
                {
                    Sonyblade._playedShing = true;
        ...
        public override void Update()
        {
            ...
            else
            {
                ...
                if (this._volatile && this._hitWait == 0)
                {
                    ...
                    bool flag3 = false;
                    using (System.Collections.Generic.IEnumerator<Thing> enumerator = Level.current.things[typeof(Sonyblade)].GetEnumerator())
                    {
                        while (enumerator.MoveNext())
                        {
                            Sonyblade sword = (Sonyblade)enumerator.Current;
            ...
            if ((this._swung || this._swinging) && !this._shing)
            {
                ...
                if (!flag4)
                {
                    using (System.Collections.Generic.IEnumerator<Thing> enumerator4 = Level.current.things[typeof(Sonyblade)].GetEnumerator())
                    {
                        while (enumerator4.MoveNext())
                        {
                            Sonyblade sword2 = (Sonyblade)enumerator4.Current;
        ...
        {
            if (base.isServerForObject && !this._crouchStance)
            {
                Level.Add(new ForceWave(base.x + (float)offDir * 6f + owner.hSpeed, base.y + 8f, offDir, 0.15f, 6f + Math.Abs(owner.hSpeed), owner.vSpeed, base.duck));
            }
