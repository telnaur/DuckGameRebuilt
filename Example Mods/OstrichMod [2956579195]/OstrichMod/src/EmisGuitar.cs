...
namespace DuckGame.OstrichMod
{
    ...
    public class EmisGuitar : Gun
    {
        ...
        public EmisGuitar(float xval, float yval) : base(xval, yval)
        {
            this.ammo = 4;
            ...
            this._type = "gun";
            base.graphic = new SpriteMap(Mod.GetPath<DuckGame.OstrichMod.OstrichMod>("EmisGuitar"), 14, 30, false);
            this.center = new Vec2(7f, 18f);
            this.collisionOffset = new Vec2(-7f, -20f);
            this.collisionSize = new Vec2(14f, 30f);
            this._barrelOffsetTL = new Vec2(7f, 0f);
            this._fireSound = "smg";
            this._fullAuto = true;
            this._fireWait = 1f;
            this._kickForce = 3f;
        ...
        public void Shing()
        {
            if (!this._shing)
            {
                ...
                if (!EmisGuitar._playedShing)
                {
                    EmisGuitar._playedShing = true;
                    this.ammo--;
        ...
        public override void Update()
        {
            if(this.ammo == 1)
            {
                base.graphic = new SpriteMap(Mod.GetPath<DuckGame.OstrichMod.OstrichMod>("BrokenEmisG"), 14, 30, false);
            }
            if(this.ammo <= 0)
            {
                Level.Remove((this));
            ...
            else
            {
                ...
                if (this._volatile && this._hitWait == 0)
                {
                    ...
                    bool flag3 = false;
                    using (System.Collections.Generic.IEnumerator<Thing> enumerator = Level.current.things[typeof(EmisGuitar)].GetEnumerator())
                    {
                        while (enumerator.MoveNext())
                        {
                            EmisGuitar sword = (EmisGuitar)enumerator.Current;
            ...
            if ((this._swung || this._swinging) && !this._shing)
            {
                ...
                if (!flag4)
                {
                    using (System.Collections.Generic.IEnumerator<Thing> enumerator4 = Level.current.things[typeof(EmisGuitar)].GetEnumerator())
                    {
                        while (enumerator4.MoveNext())
                        {
                            EmisGuitar sword2 = (EmisGuitar)enumerator4.Current;
        ...
        public override void Draw()
        {
            ...
            if ((this.owner == null && base.velocity.length > 1f) || this._swing != 0f)
            {
                ...
                for (int i = 0; i < 7; i++)
                {
                    ...
                    base.depth -= 2;
                    base.alpha -= 0.15f;
                    base.graphic.color = Color.DeepSkyBlue;
                }
